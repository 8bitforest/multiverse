...
namespace Multiverse.Serialization
{
    public static class MvSerializableTypes
    {
        ...
        public static T ReadSerializable<T>(this MvBinaryReader reader) where T : IMvSerializable
        {
            return reader.Read<T>();
        }

        public static void WriteSerializable<T>(this MvBinaryWriter writer, T value) where T : IMvSerializable
        {
            writer.Write(value);
        }
        ...
        {
            return !reader.ReadBool() ? (T?) null : reader.Read<T>();
        }
        ...
        public static MvPlayer ReadPlayer(this MvBinaryReader reader)
        {
            var id = reader.ReadUInt();
            if (id == 0)
                return null;

            return MvNetworkManager.I.HasServer
                ? MvNetworkManager.I.Server.Players[id]
                : MvNetworkManager.I.Client.Players[id];
        }

        public static void WritePlayer(this MvBinaryWriter writer, MvPlayer player)
        {
            writer.WriteUInt(player?.Id ?? 0);
        }
    }
}